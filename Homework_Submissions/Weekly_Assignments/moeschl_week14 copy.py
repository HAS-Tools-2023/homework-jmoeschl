...
pet = xr.open_dataset('pet_2020.nc')

#downloaded_files = ["pet_2020.nc", "srad_2020.nc", "vpd_2020.nc"]
...
# or print(ds[var].attrs['description'], ds.var['units'])
...
first_day['mean_vapor_pressure_deficit'].plot()
...
# TODO: Your code here
first_day['potential_evapotranspiration'].plot(figsize = (13,6), cmap = 'BrBG')
...
my_ds = ds.isel(lat=slice(0, 30), lon=slice(19, 39))
#print(random_ds)
my_ds
...
# TODO: Your code here
spatial_mean_ds = my_ds.mean(['lat', 'lon'])
...
# 
# TODO: Your code here
mine = np.corrcoef(spatial_mean_ds['potential_evapotranspiration'].values,
    spatial_mean_ds['mean_vapor_pressure_deficit'].values)

#Dr Condons (same but simpler)
np.corrcoef(
...
coarse_ds = ds.coarsen(coarse_amount, boundary='trim').mean()
coarse_ds
...
# TODO: Your code here
correlation.plot() 
plt.title('Coarsened PET and MVPD')
plt.xlabel('Longitude')
plt.ylabel('Latitude')
plt.show()
